...
export function getValues(obj: any, fieldNames: string[]) {
    // return obj.reduce((output: any, itemKey: string) => {
    //     if (fieldNames.includes(itemKey)) return { ...output, [itemKey]: obj[itemKey] };
    //     return output;
    // }, {})
    ...
    for (const key in obj) {
        if (obj.hasOwnProperty(key)) {
            ...
            if (fieldNames.includes(key)) objOutput[key] = element;
        }
    }
